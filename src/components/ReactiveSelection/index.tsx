  ...
  useEffect(() => {
    const fetchCatalogs = async () => {
      ...
      try {
        ...
        if (
          element?.solvents?.some((e) => {
            return e.name === "agua";
          })
        ) {
          setSolvent((prevState) => ({ ...prevState, Agua: true }));
        }
        if (
          element?.solvents?.some((e) => {
            return e.name === "alcohol";
          })
        ) {
          setSolvent((prevState) => ({ ...prevState, Alcohol: true }));
        }
        if (
          element?.solvents?.some((e) => {
            return e.name === "otros";
          ...
          setSolvent((prevState) => ({
            ...prevState,
            Otros: true,
            Otrosdesc: element?.solvents?.find((e) => {
              return e.name === "otros";
            })?.description!,
          }));
        }
    ...
    <div className="column">
      <div className="row">
        <FormControl variant="standard" sx={{ m: 1, minWidth: 120 }}>
          ...
          >
            {ElementsList && ElementsList.map((t) => <MenuItem value={t._id}>{t.description}</MenuItem>)}
          </Select>
        </FormControl>
        <TextField
          label="Unidades"
      ...
      <div className="row">
        <div className="semicolumn">
          ...
          <FormControl variant="standard" sx={{ m: 1, minWidth: 120 }}>
            ...
            >
              {qualityCatalog && qualityCatalog.map((t) => <MenuItem value={t.value}>{t.text}</MenuItem>)}
            </Select>
          </FormControl>
          ...
          <FormControl variant="standard" sx={{ m: 1, minWidth: 120 }}>
            ...
            >
              {TypeCatalog && TypeCatalog.map((t) => <MenuItem value={t.value}>{t.text}</MenuItem>)}
            </Select>
          </FormControl>

          <TextField
            label={"Cantidad concentración (" + (unitMeasureSelected || "un") + ")"}
            id="txtsolvente"
          ...
          <FormGroup>
            <FormControlLabel
              ...
              control={
                <Checkbox
                  onClick={() => {
                    setSolvent((prevState) => ({ ...prevState, Agua: !solventSelected.Agua }));
                  }}
            ...
            <FormControlLabel
              ...
              control={
                <Checkbox
                  onClick={() => {
                    setSolvent((prevState) => ({ ...prevState, Alcohol: !solventSelected.Alcohol }));
                  }}
            ...
            <div>
              <FormControlLabel
                ...
                control={
                  <Checkbox
                    onClick={() => {
                      setSolvent((prevState) => ({ ...prevState, Otros: !solventSelected.Otros }));
                    }}
              ...
              <TextField
                ...
                onChange={(event) => {
                  setSolvent((prevState) => ({ ...prevState, Otrosdesc: event.target.value }));
                }}
      ...
      {index > -1 && element?.id == editingId ? (
        <div>
          ...
          <Button
            variant="contained"
            hidden={idSelected != "" && Number(amountSelected) > 0}
            onClick={() => {
              onEdit(index);
            }}
            endIcon={<SendIcon />}
